...
Adding a problem touches more files than it looks like. `packages/problems/index.ts` is a **hand-maintained list** — both the web app and `apps/api`'s `/api/problems` route import the same `problems` array from it (statically, at build time). A JSON file dropped into a category directory does **nothing** on its own until it's registered there.
...
5. **Register it in `packages/problems/index.ts`**: add the import and add it to the exported `problems` array. Skipping this step means neither the web app nor `/api/problems` shows it.

6. **Update `packages/problems/index.test.ts`**: bump the `toHaveLength(N)` count, extend the sorted-ids assertion, and add/extend an `it(...)` block asserting the new id/category. `apps/api/src/routes/problems.test.ts` only checks that id 1 exists, so it doesn't need a count bump.

7. **Update the problem count in `README.md`** (currently stated as "全9問" / category breakdown).

8. Run `pnpm test` and `pnpm test:e2e` — both must stay green.
